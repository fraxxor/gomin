...
func TestProcessGofile_ImportAfterProductiveRowSkipped(t *testing.T) {
	gofile := gofilereader.Gofile{	AbsolutePath: "test/testfolder/testpackage/test.go",
											Rows: []string{"package testpackage", "doSmth()", "import(", "alias \"frax\"", ")"}}
	processor := new(ProcessGofileImpl)
	pfile, err := processor.ProcessGofile(&gofile)
	if err != nil {
		t.Errorf("Expected no error.\n")
	...
	if len((*pfile).Imports) != 0 {
		t.Errorf("Expected no import, but was <%s>\n", (*pfile).Imports)
		return
	}
}
